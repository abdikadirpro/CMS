...
import { usePartyForgotPasswordMutation, usePartyResetPasswordMutation } from "../../app/api/partyAuthApi";

export default function PartyForgotPassword() {
  const [step, setStep] = useState(1);
  const [email, setEmail] = useState("");
  const { register, handleSubmit, formState: { errors } } = useForm();
  const [forgotPassword, { isLoading: sending }] = usePartyForgotPasswordMutation();
  const [resetPassword, { isLoading: resetting }] = usePartyResetPasswordMutation();
  ...
  return (
    <div>
      ...
      <p className="mb-6 text-sm text-slate-400">
        {step === 1 ? "Enter your party admin email to receive a reset code" : `Enter the code sent to ${email}`}
      </p>
      ...
      <p className="mt-6 text-center text-sm text-slate-400">
        Remembered it?{" "}
        <Link to="/barwaaqo/login" className="font-medium text-primary hover:underline">Back to login</Link>
      </p>
    </div>
  );
}
