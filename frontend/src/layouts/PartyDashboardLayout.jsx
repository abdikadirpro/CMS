...
import { Menu, X, Moon, Sun, Flag, LogOut, ChevronDown, LayoutDashboard, UserCheck, TrendingUp, Shield, HeartHandshake, Landmark } from "lucide-react";
...
const BASE_NAV_ITEMS = [
  { to: "/barwaaqo/app", label: "Dashboard", icon: LayoutDashboard, end: true },
  { to: "/barwaaqo/app/members", label: "Members", icon: UserCheck },
  { to: "/barwaaqo/app/members/analytics", label: "Member Reports", icon: TrendingUp },
  { to: "/barwaaqo/app/volunteers", label: "Volunteers", icon: HeartHandshake },
  { to: "/barwaaqo/app/donations", label: "Donations", icon: Landmark },
];

export default function PartyDashboardLayout() {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const { mode, toggle } = useTheme();
  const { actor, partyAdminType } = usePartyAuth();
  ...
  const navItems = isSuperAdmin
    ? [
        ...BASE_NAV_ITEMS,
        { to: "/barwaaqo/app/party-branches", label: "Party Branches", icon: Flag },
        { to: "/barwaaqo/app/party-admins", label: "Party Admins", icon: Shield },
      ]
    : BASE_NAV_ITEMS;
  const roleLabel = PARTY_ADMIN_TYPE_LABELS[partyAdminType] || "Party Admin";
              ...
              trigger={
                <button className="flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-[rgb(var(--bg-alt))] transition-colors">
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary/15 text-sm font-bold text-primary">
                    {actor?.fullName?.[0]?.toUpperCase() || "P"}
                  </div>
